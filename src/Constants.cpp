//
//  Constants.cpp
//  MagIndev
//
//  Created by Brian Kelley on 11/21/14325.
//  Copyright (c) 2014 Brian Kelley. All rights reserved.
//

#include "Constants.h"

int constants::mouseX = 0;
int constants::mouseY = 0;
int constants::FONTW = -1;
int constants::FONTH = -1;
int constants::PAD = 5;
int constants::BORDER_WIDTH = 6;
int constants::DEFAULT_ATLAS_SIZE = 1024;
int constants::WINDOW_W = 640;
int constants::WINDOW_H = 480;
float constants::UI_BG_R = 0.3;
float constants::UI_BG_G = 0.3;
float constants::UI_BG_B = 0.3;
float constants::UI_FG_R = 0.7;
float constants::UI_FG_G = 0.7;
float constants::UI_FG_B = 0.7;
float constants::SHADE = 0.75;
float constants::HMULT = -80.0;